...
class CartUtils():
    ...
    def get_srv_cnt(self, cartobj):
        """ get server count """
        hostlist = cartobj.params.get("server", '/run/hosts/*/')
    ...
    def build_srv_cmd(self, cartobj, env, host, urifile=None):
        ...
        srvcmd = "{} --mca btl self,tcp -N {} --hostfile {} ".format(orterun_bin, srv_ppn, hostfile)
        if urifile is not None:
            srvcmd += "--report-uri {} ".format(urifile)
        srvcmd += env
        srvcmd += srv_ctx
        srvcmd += srv_env
        srvcmd += srv_bin
    ...
    def build_cli_cmd(self, cartobj, env, host, urifile=None):
        ...
        clicmd = "{} --mca btl self,tcp -N {} --hostfile {} ".format(orterun_bin, cli_ppn, hostfile)
        if urifile is not None:
            clicmd += "--ompi-server file:{} ".format(urifile)
        clicmd += env
        clicmd += cli_ctx
        clicmd += cli_env
        clicmd += cli_bin
